...
type Product = {
  ...
  description: string | null; image_url: string | null; stock: number; brand_id: string;
};
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      const { data: prod } = await supabase
        .from("products").select("id, name, sku, category, description, image_url, stock, brand_id")
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f8fafc]">
      <Header />
      <main className="flex-1 max-w-6xl w-full mx-auto px-5 md:px-14 py-8">
          ...
          <div className="grid md:grid-cols-2 gap-10 mt-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-4 mb-6">
                {product.stock > 0 ? <span className="text-sm text-green-600 font-medium">● In stock</span> : <span className="text-sm text-slate-400 font-medium">● Backorder</span>}
              ...
              <div className="bg-white border border-slate-200 rounded-md p-5 mb-6">
                ...
                <a href="/login" className="inline-block bg-[#0d2b5e] text-white text-xs uppercase tracking-wider px-6 py-3 rounded-sm hover:bg-[#163d80] transition-colors">Login for pricing</a>
              </div>
              {product.description && (
                <div className="mb-6">
