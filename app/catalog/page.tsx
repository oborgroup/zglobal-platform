...
function thumb(url: string | null): string {
  ...
  if (url.includes("cdn.shopify.com")) {
    return url.replace(/(\.[a-zA-Z]+)(\?|$)/, "_400x$1$2");
  }
  return url;
}
            ...
            <>
              {activeBrand === "all" ? (
                  ...
                  return (
                    <div key={b.id} className="mb-12">
                      <div className="flex items-baseline justify-between mb-4 border-b border-slate-200 pb-2">
                        <h2 className="text-lg font-semibold text-[#0d2b5e]">{b.name}</h2>
                        <span className="text-xs text-slate-400">{brandProducts.length} products</span>
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {brandProducts.map((p) => <Card key={p.id} p={p} brandName={b.name} />)}
                      </div>
                    </div>
                  );
                })
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {filtered.map((p) => <Card key={p.id} p={p} brandName={brandName(p.brand_id)} />)}
                  </div>
                </>
              )}
            </>
